...
#main function
def main(input_path_test, downsampling_fact, downsampling_mode, channels, data_format, label_id, blocks, weights, image_dir, checkpoint_dir, output_graph_file, tst_sz, loss_type, fs_type, batch, batchnorm, dtype, filter_sz, growth, scale_factor, predmode):
    ...
    with test_graph.as_default():
        #create readers
        tst_reader = h5_input_reader(input_path_test, channels, weights, dtype, normalization_file="stats.h5", update_on_read=False, label_id=label_id, read_labels=(not predmode))
        #create datasets
        if fs_type == "local":
            ...
            if downsampling_mode == "scale":
                ...
                if data_format=="channels_first":
                    if not predmode:
                        ...
                        next_elem = (tf.transpose(next_elem[0], perm=[0,2,3,1]), next_elem[1])
                ...
                if data_format=="channels_first":
                    if not predmode:
                        ...
                        next_elem = (tf.transpose(next_elem[0], perm=[0,3,1,2]), next_elem[1])
                ...
                try:
                    ...
                    if have_imsave:
                        ...
                        if not predmode:
                            ...
                            np.savez(image_dir+'/test_estep'
                                     +str(eval_steps)+'_rank'+str(comm_rank)+'.npz', prediction=np.argmax(tst_model_predictions[...],axis=-1)*100, filename=tst_model_filenames)
